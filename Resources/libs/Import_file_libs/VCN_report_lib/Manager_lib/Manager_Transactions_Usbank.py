...
from VCN_report_lib.Transaction_lib import Transaction_Usbank

class ManagerTransactionsUsbank(Manager):
    def expected_trans(self, itype, iinfo, iindice):
        list_type = itype.split('_')
        if list_type[0] == 'USB':

            if list_type[1] == "P" :
                return #Transaction_Usbank.TransactionIxarisCREATED(itype, iinfo, iindice)
            elif list_type[1] == "C" :
                return #Transaction_Usbank.TransactionIxarisDELETED(itype, iinfo, iindice)
